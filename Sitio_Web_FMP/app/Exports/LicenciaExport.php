...
class LicenciaExport implements FromView{
    ...
    public function __construct(/*$tipo_contrato,*/ $anio, $mes, $departamento, $comentario){
        ...
        $this->mes_ant = ($this->mes>1?$this->mes-1:$this->mes);
    ...
    */
    ...
    public function view(): View{
        ...
        and to_char(p1.fecha_uso,'YYYY')::int=".$this->anio." and to_char(p1.fecha_uso,'MM')::int=".$this->mes."

        left join permisos p2 on p2.empleado = e.id and p2.estado like 'Aceptado' and p2.tipo_permiso like 'LS/GS'
        and to_char(p2.fecha_uso,'YYYY')::int=".$this->anio." and to_char(p2.fecha_uso,'MM')::int=".$this->mes."
        ...
        and to_char(p3.fecha_uso,'YYYY')::int=".$this->anio." and to_char(p3.fecha_uso,'MM')::int=".$this->mes."

        left join permisos p4 on p4.empleado = e.id and p4.estado like 'Aceptado' and p4.tipo_permiso like 'T COMP'
        and to_char(p4.fecha_uso,'YYYY')::int=".$this->anio." and to_char(p4.fecha_uso,'MM')::int=".$this->mes."
        ...
        and to_char(p5.fecha_uso,'YYYY')::int=".$this->anio." and to_char(p5.fecha_uso,'MM')::int=".$this->mes."
        ...
        and to_char(p6.fecha_uso,'YYYY')::int=".$this->anio." and to_char(p6.fecha_uso,'MM')::int=".$this->mes."

        left join permisos p7 on p7.empleado = e.id and p7.estado like 'Aceptado' and
        p7.tipo_permiso like 'Const. olvido' and to_char(p7.fecha_uso,'YYYY')::int=".$this->anio." and to_char(p7.fecha_uso,'MM')::int=".$this->mes."

        left join permisos p8 on p8.empleado = e.id and p8.estado like 'Aceptado' and
        p8.tipo_permiso like 'DUELO O PATERNIDAD' and to_char(p8.fecha_uso,'YYYY')::int=".$this->anio." and to_char(p8.fecha_uso,'MM')::int=".$this->mes."
    ...
    /*select TRIM(e.apellido)||','||TRIM(e.nombre) as nombre,null as hrs_inpunt,null as hrs_inasis,null as hrs_descont,
       ...
       tipo_permiso like 'LC/GS' and to_char(fecha_uso,'MM-YYYY') like '2021-10' where e1.id=e.id),
