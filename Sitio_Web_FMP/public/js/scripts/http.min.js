    ...
    $.ajax({
        ...
        if(data.error!=null){
            ...
            if(data.mensaje!=null && data.error==null){
                $(notificacion).removeClass().addClass('alert alert-success bg-success text-white border-0').show().html(
                    +'<div class="row">'
                        +'<div class="col-lg-9 order-firts">'
...
function submitActivarDesactivar(boton){
    $.ajax({
        type: $(formulario).attr('method'), 
        url: $(formulario).attr('action'),
        data: $(formulario).serialize(),
...
$('.modal').on('hidden.bs.modal',function(){$(".alert").hide();$(".summernote-config").summernote("code","");$("form").trigger("reset");});