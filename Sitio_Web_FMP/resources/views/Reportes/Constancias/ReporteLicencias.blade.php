    ...
    <table id="const" class="table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{''.\Carbon\Carbon::parse($item->fecha_uso . 'T' . $item->hora_inicio)->diffAsCarbonInterval(\Carbon\Carbon::parse($item->fecha_uso . 'T' . $item->hora_final))}}</span></td>
                <td>{{rtrim(mb_strimwidth(strip_tags($item->justificacion), 0, 125, '', 'UTF-8'))}}</td>
            </tr> 
        @endforeach 
